...
def create_option_parser(parser):
    ...
    parser.add_option('--brute', help='brute force attack',
                      dest='attack_brute', default=False, action='store_true')
    parser.add_option(
        '--min', help='minimum chars testing (default : 1)', dest='min', default=4)
    parser.add_option(
        '--max', help='maximum chars testing (default : 1)', dest='max', default=6)
    parser.add_option('--Charset', help=options_charset,
                      dest='charset', default="%s" % (string.ascii_letters))
    parser.add_option(
        '--resume', help='Resume to a password attempt', dest='resume', default=None)
    parser.add_option('--dico', help='dictionnary force attack',
                      dest='attack_dico', default=False, action='store_true')
    parser.add_option(
        '--file', help='File for the dictionnary attack', dest='file_dico', default=None)
    parser.add_option(
        '--id', help='id of the form', dest='form_id', default=None)
    parser.add_option(
        '--name', help='name of the form', dest='form_name', default=None)
    parser.add_option(
        '--action', help='URL action', dest='form_action', default=None)
    parser.add_option(
        '--m', help='Fail message when login failed', dest='message_failed')
    parser.add_option('--p', help='Parameter to crack', dest='param_to_crack')
    parser.add_option(
        '--cookie', help='Cookie that you want in the header', dest='cookie')
    parser.add_option(
        '--t', help='Number of thread for all the attacks', dest='num_threads', default=10)
    parser.add_option(
        '--g', help='Names of the generated inputs separated with commas',
        dest='generated', default=False, action='store_true')
    parser.add_option('--user_agent', help='Spoof the User-Agent',
                      dest='user_agent', default="Burito Cracker v0.1")
    parser.add_option('--log', help='LOG file', dest='log_file')
    parser.add_option(
        '--status-code', help='status code accepted for the response', dest='accepted_status_code')
    parser.add_option(
        '--post-data', help='If generated value, POST params to access page', dest='post_params', default=None)
...
def check_valid_parameters_brute():
    ...
    GlobalVars.opts.charset = GlobalVars.opts.charset.replace(
        '[0-9]', string.digits)
    GlobalVars.opts.charset = GlobalVars.opts.charset.replace(
    ...
    if (GlobalVars.opts.resume is not None):
        if (len(GlobalVars.opts.resume) != GlobalVars.opts.min):
            raise Exception(
                "Password resume should be same length than minimum !")
